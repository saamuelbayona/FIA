    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        >
          <div className="flex items-center justify-between mb-2">
            <span className="text-gray-400 text-sm">Total Activos</span>
            <TrendingUp className="w-5 h-5 text-blue-400" />
          </div>
          <div className="text-2xl font-bold text-white mb-1">{formatCurrency(totalActivos2025)}</div>
          <div className="text-sm text-gray-400">2024: {formatCurrency(totalActivos2024)}</div>
        ...
        >
          <div className="flex items-center justify-between mb-2">
            <span className="text-gray-400 text-sm">Total Pasivos</span>
            <TrendingDown className="w-5 h-5 text-red-400" />
          </div>
          <div className="text-2xl font-bold text-white mb-1">{formatCurrency(totalPasivos2025)}</div>
          <div className="text-sm text-gray-400">2024: {formatCurrency(totalPasivos2024)}</div>
        ...
        >
          <div className="flex items-center justify-between mb-2">
            <span className="text-gray-400 text-sm">Patrimonio</span>
            <DollarSign className="w-5 h-5 text-green-400" />
          </div>
          <div className="text-2xl font-bold text-white mb-1">{formatCurrency(totalPatrimonio2025)}</div>
          <div className="text-sm text-gray-400">2024: {formatCurrency(totalPatrimonio2024)}</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <motion.div
          ...
          className="bg-slate-800/50 backdrop-blur-xl rounded-xl p-6 border border-slate-700"
        >
          <h3 className="text-lg font-semibold text-white mb-4">Principales Activos (2025 vs 2024)</h3>
      ...
      >
        <h3 className="text-lg font-semibold text-white mb-4">Balance General Detallado - Comparativa 2024 vs 2025</h3>
        <div className="space-y-6">
          {['ACTIVOS', 'PASIVOS', 'PATRIMONIO'].map((tipo) => {
            ...
            return (
              <div key={tipo}>
                <h4 className="text-md font-semibold text-blue-400 mb-2">{tipo}</h4>
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-slate-700">
                      <th className="text-left py-2 px-4 text-gray-400">Categoría</th>
                      <th className="text-right py-2 px-4 text-gray-400">2024</th>
                      <th className="text-right py-2 px-4 text-gray-400">2025</th>
                    ...
                    {items.map((row, idx) => {
                      ...
                      return (
                        <tr key={idx} className="border-b border-slate-700/50 hover:bg-slate-700/30">
                          <td className="py-2 px-4 text-white">{row.categoria}</td>
